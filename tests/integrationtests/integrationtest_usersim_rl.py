...
if __name__ == "__main__":
    ...
    for domain in [JSONLookupDomain(domain_str) for domain_str in ['ImsCourses']]:
        ...
        success[domain.name] = []
            ...
            noise = SimpleNoise(domain=domain, train_error_rate=0.30, test_error_rate=0.30)
            policy = DQNPolicy(domain=domain, lr=0.0001, eps_start=0.3, gradient_clipping=5.0,
                               buffer_cls=RankPrioritizedBuffer, replay_buffer_size=20000, shared_layer_sizes=[256],
                               train_dialogs=TRAIN_EPISODES, target_update_rate=3, training_frequency=2, logger=logger)
            # policy = HandcraftedPolicy(domain=domain, logger=logger)
            evaluator = PolicyEvaluator(domain=domain, use_tensorboard=True, experiment_name="Rank_Prioritized", logger=logger)
